...
({
    isTaggedCall: firstArgument =>
        isArray(firstArgument) &&
        hasOwnProperty.call(firstArgument, "raw"),
...
const type = (...arguments) =>
    template.isTaggedCall(...arguments) ?
        (...definition) =>
            type (template.resolve(...arguments), ...definition) :
    given((
        anonymous = typeof arguments[0] !== "string",
        name = anonymous ? false : arguments[0],
        rest = anonymous ? arguments : arguments.slice(1)) =>
        /*false ? :*/
        data(name, rest));

    typeof arguments[0] === "string" ?
        console.log("NAMED: " + arguments[0]) :
        console.log("ANONYMOUS" + arguments);

module.exports = type;

function TypeExpression(...args)
{
   if (!(this instanceof term))
        return (...rest) => new term(args[0][0], ...rest);
...
Type.Expression.Sum = Type.Expression `Sum` (
    ...
    given((nonSumSatisfies = (criterion, candidate) =>
        given(({ expanded, expandedSet } = criterion[Definition]) =>
            ...
            expanded.some(summand => satisfies(summand, candidate)))) =>
        (criterion, candidate) =>
...
Type.Expression.Function = Type.Expression `Function` (
    ...
    (TF, definition, args) => Type.Expression.Invocation(
...
Type.Expression.Invocation = Type.Expression `Invocation` (
    ...
    (criterion, candidate) =>
        ...
        given((
            ...
            lDefinition.arguments.length === rDefinition.arguments.length &&
            lDefinition.arguments.every((argument, index) =>
    ...
    function (NominalT, definition, args)
    {
        return definition
            ...
            .construct
            .call(this, NominalT, definition.effective[Definition], args);
    });
